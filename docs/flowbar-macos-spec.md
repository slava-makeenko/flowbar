...
**Экран «Лимиты».** Пятый пункт рейла. Для Claude Code и Codex — окна лимита (5 часов, неделя): **остаток** («осталось 32 %»), время сброса, полоса остатка — убывает по мере расхода, возраст снимка. Полосы — в цвете агента, как его точка в пилюле; при остатке 10 % и меньше подпись и полоса — `danger`.
...
Папки `~/.claude` и `~/.codex` читаются напрямую (ADR-0016); нет папки — «не установлен?» и кнопка «Проверить снова». Лимиты Claude — живой запрос `get_usage` у самого Claude Code при открытии экрана, по ↻ и раз в минуту, пока экран открыт; снимок statusLine — запасной, побеждает более свежий. Файлы читаются при открытии экрана и раз в 10 с, пока он открыт; кнопка «Перечитать лимиты» внизу экрана читает их сразу, спрашивает Claude Code заново и подтверждает тостом. Если агент работал позже снимка больше чем на 2 минуты, под заголовком — пометка «Устарело», прогноз и совет по такому снимку не показываются (ADR-0012: вкладка Code десктопного приложения statusLine не запускает).

**Индикатор в пилюле.** Агент считается работающим, пока в его сессии идёт ход и последняя запись в транскрипт (`~/.claude/projects`, `~/.codex/sessions`) моложе 20 с; маркер конца хода гасит точку сразу (ADR-0014). Наблюдение — FSEvents. Codex держит rollout открытым, и FSEvents его записей не видит, поэтому работа Codex видна через его хуки: кнопка «Подключить» на экране «Лимиты» ставит в `~/.codex/hooks.json` хук на `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, который пишет событие в `~/.codex/flowbar/<session_id>.json`. Одобряет хуки пользователь в самом Codex (ADR-0017). Точка: Claude Code — `#D97757`, Codex — `#4C8DFF`; оба — столбиком, Claude сверху. Только в свёрнутом виде. Никто не работает — в крыле пусто. Без выданного доступа индикатор молчит.
...
> **Закрыта ADR-0016:** Developer ID без песочницы. Причина — живые лимиты Claude через `get_usage`: под песочницей запущенный `claude` не видит ни `~/.claude`, ни Keychain, ни сети. Entitlements пусты. Папка снимков без выбора — системная (`com.apple.screencapture`), выбор пользователя по-прежнему помнит закладка. Данные прежнего контейнера переносит `SandboxContainerMigration` при первом запуске.
...
- [ ] Работа агента зажигает его точку в пилюле не позже чем через 2 с; конец хода гасит её за 1–2 с, тишина посреди хода — за 20 с
